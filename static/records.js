...
class RecordsList extends React.Component {
  ...
  render() {
    ...
    this.state.records.forEach(record => {
      try {
        if (!Object.keys(filters).every(key => record[key].toString().match(new RegExp(filters[key], "i")))) return;
      } finally {}

      records.push(React.createElement(RecordsRow, {
        key: `${record.id}-${record.f_time}`,
        record: record,
        update: this.update.bind(this)
      }));
    });
    return [React.createElement(RecordsHeader, {
      key: "header"
    }), React.createElement(RecordsSearchbar, {
      key: "searchbar",
      update: this.update.bind(this),
...
class RecordsSearchbar extends React.Component {
  ...
  render() {
    ...
    }, fields.map(([name, field]) => React.createElement("div", {
      key: name,
      className: "col-sm-2 col-record"
...
class RecordsHeader extends React.Component {
  render() {
    ...
    }, names.map(name => React.createElement("div", {
      key: name,
...
class RecordsRow extends React.Component {
  render() {
    ...
    if (record.type == 'image') {
      ...
      data = data.split('\n').map((text, key) => React.createElement("span", {
        key: key
      }, text, React.createElement("br", null)));
    } else if (record.type == 'function') {
      data = React.createElement("pre", null, React.createElement("code", {
        className: "",
    ...
    return React.createElement("div", {
      ...
      key: field,
      className: "col-sm col-record"
    }, React.createElement("p", null, record[field]))), React.createElement("div", {
      className: "col-sm-4 col-record"
    }, data));
  }
